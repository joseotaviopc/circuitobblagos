...
export function HomeAtletas({ atletas: propAthletes }: HomeAtletasProps) {
	...
	const categoriesMap = new Map<
		string,
		(Atleta & { totalPoints: number; bestPosition: number })[]
	>();
	...
	athletesToUse.forEach((athlete) => {
		athlete.resultados?.results.forEach((res) => {
			if (res.posicao !== 0) {
				...
				const existingAthleteIndex = currentCategoryAthletes.findIndex(
					(a) => a.id === athlete.id,
				);
				...
				if (existingAthleteIndex > -1) {
					...
					existingAthlete.bestPosition = Math.min(
						existingAthlete.bestPosition,
						res.posicao,
					);
				} else {
	...
	const topAthletesByCategory = Array.from(categoriesMap.entries())
			...
			<Card>
							...
							<li key={category}>
								<div className="flex flex-wrap items-center gap-4 rounded-lg border p-2 justify-between">
									<p className="flex-1 font-semibold text-xs text-nowrap uppercase tracking-wider text-muted-foreground">
										{category}{" "}
										{category.toLowerCase().includes("fem") ? "🚺" : "🚹"}
									</p>
									<Link
				...
				<CardFooter className="p-4 pt-0">
					<Button
						asChild
						variant="link"
						className="p-0 h-auto w-full justify-start md:w-auto"
					>
						<Link href="/atletas">
							Ver todos
