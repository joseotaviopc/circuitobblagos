...
function RankingsContent() {
	...
	const allCategories = Array.from(
		new Set(
			atletas.flatMap(
				(atleta) =>
					atleta.resultados?.results.map((res) => res.categoria) || [],
			),
		),
	);
	const [activeTab, setActiveTab] = useState<string>(
		(category as string) || allCategories[0],
	);
	const router = useRouter();

	const filteredAthletes = atletas
		.map((athlete) => {
			const resultsInActiveCategory = athlete.resultados?.results.filter(
				(res) => res.categoria === activeTab && res.posicao !== 0,
			);
			if (resultsInActiveCategory && resultsInActiveCategory.length > 0) {
				...
				const totalPoints = resultsInActiveCategory.reduce(
					(sum, res) => sum + res.pontos,
					0,
				);
				// Find the best position (lowest number) for the athlete in the active category
				const bestPosition = Math.min(
					...resultsInActiveCategory.map((res) => res.posicao),
				);
		...
		<div className="space-y-8">
			<header>
				...
				<p className="mt-2 text-lg text-muted-foreground">
					Confira o ranking oficial dos atletas de bodyboard em todas as
					categorias.
				</p>
			</header>
			<Card>
								...
								{filteredAthletes.length > 0 ? (
									filteredAthletes.map((athlete, index) => (
										<TableRow key={athlete!.id}>
											<TableCell className="font-bold text-lg text-center p-1 md:p-4">
												{index + 1}
											</TableCell>
											<TableCell className="p-1 md:p-4">
												...
												>
													...
													<span className="font-medium group-hover:text-primary transition-colors text-sm md:text-base">
														{athlete!.nome}{" "}
														{athlete?.estado && ` (${athlete?.estado})`}
														{athlete && athlete.isAffiliated === "true" && (
															<CheckCircle className="ml-1 h-4 w-4 text-green-500 inline-block" />
														)}
													</span>
												</Link>
											</TableCell>
											<TableCell className="text-right font-semibold text-primary p-1 pr-2 md:p-4 md:pr-4">
